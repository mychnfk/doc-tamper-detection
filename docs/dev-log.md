...
## 初赛 Day 1 实测补录（2026-06-30 测得，决赛阶段补录存档）

> 以下两项为初赛 Day 1（2026-06-30）的真实实测结果。当时本仓库尚无 dev-log（本文件创建于决赛升级 Task 1，2026-07-19），原始记录仅存于当日工作会话；决赛 QA 预案需要可指认的仓库内出处，故在此补录存档。补录属性如实标注，不冒充当时的即时记录。

### ① 缩放实验：图像缩放摧毁 Noiseprint++ 特征（坑 #2 铁律的来源）
同一张篡改图仅缩小 23%（1080px→830px），TruFor 篡改检出分数从 0.945 跌至 0.177。机理：LANCZOS 重采样把相邻像素的噪声模式混合在一起，直接抹掉 Noiseprint++ 赖以工作的高频相机噪声指纹。结论固化为坑 #2 铁律：送入检测器的图像绝不缩放，超限走切片（见 `docs/superpowers/plans/2026-07-19-agent-loop-finals.md` Global Constraints 第 14 行）。

### ② MPS 分辨率上限：2048px OOM → MAX_SIZE=1792 的来源
M3 8GB 统一内存下，SegFormer-B2 的 self-attention 在 2048px 输入时 OOM。据此将单次推理上限定为 1792px（今 `config.py` 的 `DOCGUARD_MAX_SIZE` 默认值），超过该尺寸的图片走 `run_tiled()` 切片推理；服务器（16G VRAM）到位后按附录服务器轨清单逐档提测 2048→2560→3072。
